...
def run_analyze(har_path: str, verbose: bool = False, debug: bool = False,
                ...
                quiet: bool = False, debug_schema: bool = False,
                debug_chatgpt: bool = False) -> dict:
    """
    ...
    if not quiet:
        for section in ["A_identity", "B_prompt", "C_security", "D_autonomous", "E_urls"]:
            print_section_results(section, results.get(section, []), verbose=verbose)
    ...
    if export_fmt:
        ...
        if export_fmt == "json":
            ...
            exported = export_csv(results, platform, har_path, output_dir, debug=debug_schema)
        elif export_fmt in ("xlsx", "excel"):
            p = export_excel(results, platform, har_path, output_dir,
                             cg, gm, cl, debug=debug_schema)
            exported = [p]
            ...
            all_exports = export_all(results, platform, har_path, output_dir, cg, gm, cl, debug=debug_schema)
...
def cmd_analyze(args: argparse.Namespace) -> None:
    ...
    # Activate strict schema mode when --debug-schema is set
    debug_chatgpt = getattr(args, "debug_chatgpt", False)
    debug_schema = getattr(args, "debug_schema", False)
    if debug_schema:
        from parsers.schemas import set_strict_mode, print_schema_report
        set_strict_mode(False)   # log violations without aborting
        print_schema_report()    # dump all section schemas
...
Examples:
  ...
  python main.py validate session.har
    ...
    analyze_p.add_argument(
        ...
        choices=["json", "csv", "xlsx", "excel", "md", "markdown", "txt", "text", "all"],
        help="Export format: json, csv, xlsx, md, txt, or all",
    )
    analyze_p.add_argument(
    ...
    analyze_p.add_argument(
        "--debug-chatgpt", action="store_true", dest="debug_chatgpt",
        help=(
            "Print detailed ChatGPT HAR forensics debug output: detection, endpoint matches, "
            "SSE events, artifact counts, and missed artifact report."
        ),
    )
    analyze_p.add_argument(
        "--debug-schema", action="store_true", dest="debug_schema",
        help=(
            "Print per-section schema, rejected fields, enrichment pipeline, "
            "and attribution source for each exported section."
        ),
    )
    ...
    batch_p.add_argument(
        "--export", "-e", metavar="FORMAT",
        choices=["json", "csv", "xlsx", "excel", "md", "txt", "all"],
