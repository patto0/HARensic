...
def _chatgpt_B_prompt(entries: List[Dict], sse: List[Dict]) -> List[Dict]:
    ...
    for model in models_seen:
        rows.append({"artifact":"model_used","value":model,
                     ...
                     "json_path":"request.postData.text[model]","attribution":"HUMAN",
    ...
    for evt in sse:
        data = evt.get("data",{})
        if not isinstance(data,dict): continue
        if data.get("o") == "append" and "content/parts" in str(data.get("p","")):
            val = data.get("v","")
            if isinstance(val,str): token_buffer.append(val)
    if token_buffer:
        rows.append({"artifact":"ai_generated_response","value":"".join(token_buffer),
                     "har_location":f"SSE delta append ({len(token_buffer)} tokens)",
                     "json_path":"SSE delta[o=append][p=content/parts][v]","attribution":"AI",
        ...
        rows.append({"artifact":"ai_token_count","value":str(len(token_buffer)),
                     "har_location":"SSE delta append operations",
                     "json_path":"SSE delta[o=append] count","attribution":"AI",
